...
public class AdminMemberController {
    ...
    public String addForm(Model model){
        ...
        return "admin/member/addForm"; //가입화면
    }
    ...
    //검증
    //model.addAttribute("addForm",addForm); @ModelAttribute 어노테이션 사용으로인해 안해도 된다
    log.info("addForm={}",addForm);
...
//    }
    ...
    public String edit(@PathVariable("id") Long id,
        ...
        int updateRow = adminMemberSVC.update(id,member);
        if(updateRow == 0) {
            return "admin/member/editForm";
        }
    ...
    public String del(@PathVariable("id") Long id){
        int deletedRow = adminMemberSVC.delete(id);
        if(deletedRow == 0){
            return "redirect:/admin/members/"+id; //회원 상세화면
        }
